...
public ResponseDTO<PmCaseDTO> getPmCaseInfo(int caseId) {
		...
		String mtItems = caseMapper.getMaintainItems(mtCase);

		mtCase.setPmTemplate(mtItems);
...
public ResponseDTO<PmCaseDTO> getPmCaseInfoWithDev(int deviceId) {
	
	ResponseDTO<PmCaseDTO> res = new ResponseDTO<PmCaseDTO>();

	PmCaseDTO mtCase = caseMapper.getMainCaseInfoWithDev(deviceId);
	...
	// 取得设备保养项目
	public ResponseDTO getMaintainItems(PmCaseDTO caseDto) {
	
		ResponseDTO res = new ResponseDTO();
	
		String mtItems = caseMapper.getMaintainItems(caseDto);
		
		if (mtItems == null) {
			res.code = ConstantInfo.INVALID;
			res.message = "没有该设备的保养项目信息!";
			return res;
		}
...
public ResponseDTO completePmCase(PmCaseDTO caseDto) {
	...
	if (StringUtils.isEmpty(caseDto.getPmResult())) {
		res.code = ConstantInfo.INVALID;
		res.message = "未填写保养项目!";
		return res;
	}
	...
	try {
		...
		int count = 0;
		Integer caseId = caseMapper.getPmCaseId(caseDto.getDeviceId());
		if (caseId != null) {
