	...
	public ResponseDTO getUsersWithRole(Integer roleId) {
	   ...
	   return res;
		...
		public ResponseDTO getUsersWithoutRole(Integer roleId) {
			...
			try {
				List<UserRole> roleInfos = userMapper.getUsersWithoutRole();
		...
		// 取得部门用户
		public ResponseDTO getUsersWithDept(Integer deptId) {
			...
			try {
				List<UserRole> roleInfos = userMapper.getUsersWithDept(deptId);
		   ...
		   return res;
			...
			// 取得 未分配部门的用户
			public ResponseDTO getUsersWithoutDept(Integer deptId) {
				
				ResponseDTO res = new ResponseDTO();
				
				
				try {
					List<UserRole> roleInfos = userMapper.getUsersWithoutDept();
		...
		@Transactional(propagation = Propagation.REQUIRED,isolation = Isolation.DEFAULT,timeout=36000,rollbackFor=Exception.class)
		public ResponseDTO confirmUserToDept(User userInfo) {
			...
			List<UserRole> oldURInfos = userMapper.getUsersWithDept(deptId);
			...
			for (UserRole oldUser : oldURInfos) {
				if (!users.contains(oldUser)){
					// 添加新用户到此角色 
					userMapper.deleteUserFromDept(oldUser);
				}
